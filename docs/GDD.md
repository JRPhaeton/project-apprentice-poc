...
| 11 | 2026-07-18 | Art bible / PLAN §3 | M11 "Modern 2D" (user direction: Chained Echoes register): SNES ≤16-color cap retired — new budgets 48/sheet sprites, 64 tileset pool, 96 backdrops, hue-shifted shadow ramps; WebGL postFX layer (bloom on emissives, per-room ColorMatrix grading, vignette); parallax battle backdrops. 16px grid + all dims unchanged (CI stable); contracts/core untouched. | Orchestrator |
| 10 | 2026-07-17 | §3/§4/§7 | M10 "Vale Alive": 7 treasure chests (herbs/moss/draughts/bottle) + Keeper NPC placed via additive compositor extras (collide grids + v1 objects preserved, asserted); per-room ambient particles; Victory Emberheart relight beat; chest/level-up SFX; level-ups now actually apply on battle victory (pre-existing bug: applyVictoryXp was sim-only). | Orchestrator |
| 9 | 2026-07-17 | Contracts/§5 | M10 contract amendment (orchestrator PR per PLAN §7): effect kind `restoreMp` + BattleEvent `mpRestored`; new item Mana Moss (+6 MP), 1× starting. Goldens/sim unaffected (frozen data + item-less careless policy). | Orchestrator |
| 8 | 2026-07-17 | §5 | M9 survivability items (player feedback: stage too attritive for human play): herbs 2→4, NEW item Ember Draught (+30 HP, 1×, story-flavored). Careless sim policy uses no items, so all §10 lethality gates hold (verified: 92/92 incl. balance sim); tactical margin widens. | Orchestrator |
