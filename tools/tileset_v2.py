...
# ---------------------------------------------------------------------------
# Master palette — same <= 32-color pool as the M6 sheet (ART_BIBLE §2).
...
def t_grass_b():
    return grass_tile(T["grass"], T["grass_lt"], T["grass_dk"], 4, n_blades=5)
...
def t_water_full(phase=0):
    ...
    dither(px, 0, 14, 15, 15, T["void"], 4, ox=3, only=T["water_dk"])
    s = phase * 2
...
def _boulder(t, salt=0):
    ...
    # facets: lit NW planes, shaded SE base
    ...
    px[6 + ox, 11] = T["stone_dk"]
    px[10 + ox, 13] = T["stone_dk"]
...
def _canopy_carpet():
    ...
    for (cx, cy, r) in _LOBES:
        for ox, oy in offs:
            ...
            d.ellipse([x0 - r + 2, y0 - r + 2, x0 - r // 2, y0 - r // 2], fill=T["grass_lt"])
...
def _trunk(base_fn):
    ...
    d.line([(10, 0), (10, 15)], fill=T["mud_dk"])
...
def t_wall_face():
    ...
    # base contact shadow
    d.rectangle([0, 14, 15, 15], fill=T["stone_dk"])
...
def t_wall_top():
    ...
    # side rims: lit W, shaded E
    d.line([(0, 0), (0, 15)], fill=T["stone_lt"])
    d.line([(15, 0), (15, 15)], fill=T["stone_dk"])
...
def t_gate_face():
    ...
    # timber lintel remnant across the cap
    d.rectangle([1, 0, 14, 0], fill=T["trunk_lt"])
    # ember lantern bracket
    px[12, 5] = T["trunk"]
    px[12, 6] = T["ember"]
    px[12, 7] = T["ember_lt"]
    px[11, 7] = T["ember_dk"]
...
def t_ruin_top():
    ...
    d.line([(14, 0), (14, 15)], fill=T["stone_dk"])
    return t
...
def t_cliff_face():
    ...
    dither(px, 0, 14, 15, 14, T["stone_dk"], 6)
    return t
...
def t_shadow_grass():
    return _shadowed(t_grass_a(), T["grass_dk"], T["canopy_dk"])


def t_shadow_dark():
    return _shadowed(t_dark_grass(), T["canopy_dk"], T["void"])


def t_shadow_mud():
    return _shadowed(t_mud(), T["mud_dk"], T["void"])


def t_shadow_ruin():
    return _shadowed(t_ruin_floor(), T["stone_dk"], T["void"])


def t_shadow_path():
    return _shadowed(t_path_full(), T["sand_dk"], T["mud_dk"])
